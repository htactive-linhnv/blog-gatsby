...
export default props => {
  const codingPosts = props.data.filter(({ node }) =>
    node.frontmatter.categories.includes("Coding")
  )
  const sidePosts = props.data.filter(({ node }) =>
    node.frontmatter.categories.includes("Chuyện bên lề")
  )
  const codingList = Array.from(
    new Set(
      Array.from(
        codingPosts.map(({ node }) => node.frontmatter.categories)
      ).reduce((a, b) => a.concat(b), [])
    )
  )
  const sideList = Array.from(
    new Set(
      Array.from(
        sidePosts.map(({ node }) => node.frontmatter.categories)
      ).reduce((a, b) => a.concat(b), [])
    )
  )
      ...
      <div className="menu__wrapper">
        ...
        >
          ...
          <Col span={17}>
            <Menu
              mode="horizontal"
              style={{ textAlign: "right", fontSize: "1.1em", border: "none" }}
            >
              <Menu.Item key="home">
                <Icon type="home" />
                <Link
